...
export function ServicePageTemplate() {
  ...
  if (!service) {
    return (
      <div className="bg-[#F8FAFA] pt-32 pb-20 px-6 text-center">
        <h1 className="font-cormorant text-2xl text-[#102A32] mb-4">Palvelua ei löytynyt</h1>
        <Link to="/" className="font-inter text-[13px] text-[rgba(16,42,50,0.7)] hover:text-[#123F3D]">
          Takaisin etusivulle
        </Link>
      </div>
    );
  ...
  return (
    <div className="bg-[#F8FAFA] min-h-[100dvh]">
      <nav className="fixed top-0 left-0 right-0 z-50 bg-[#123F3D] shadow-[0_1px_12px_rgba(0,0,0,0.15)]">
        <div className="max-w-[1200px] mx-auto px-5 md:px-10 h-[60px] md:h-[68px] flex items-center justify-between">
          <Link to="/" className="relative z-10">
            <img src="/assets/hk_logo.jpg" alt="Helsinki Kiropraktiikka" className="h-8 md:h-9 w-auto" />
          </Link>
          <Link to="/" className="inline-flex items-center gap-1.5 font-inter text-[13px] font-semibold uppercase tracking-[0.12em] text-[#C7DEE4]/90 hover:text-[#00B8B5] transition-colors">
      ...
      <section className="relative pt-[120px] md:pt-[140px] pb-16 md:pb-24 px-6 md:px-12">
        <div className="max-w-[1200px] mx-auto">
          <ScrollReveal>
            <div className="mb-6">
              <Link to="/#palvelut" className="inline-flex items-center gap-1.5 font-inter text-[12px] font-semibold uppercase tracking-[0.12em] text-[rgba(16,42,50,0.7)] hover:text-[#123F3D] transition-colors">
                <ArrowLeft size={14} strokeWidth={1.5} />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10 md:gap-16 items-start">
            <div>
              <ScrollReveal>
                <p className="font-inter text-[11px] font-semibold uppercase tracking-[0.2em] text-[rgba(16,42,50,0.7)] mb-4">{service.eyebrow}</p>
                <h1 className="font-cormorant text-[30px] md:text-[38px] text-[#102A32] leading-[1.25] mb-4">{service.title}</h1>
                <p className="font-inter text-[15px] text-[rgba(16,42,50,0.7)] leading-[1.7] mb-8">{service.description}</p>

                <div className="mb-8">
                  <p className="font-inter text-[11px] font-semibold uppercase tracking-[0.12em] text-[rgba(16,42,50,0.7)] mb-3">Hyödyt</p>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    {service.benefits.map((b) => (
                      <div key={b} className="flex items-center gap-2">
                        <ArrowRight size={12} strokeWidth={1.5} className="text-[rgba(16,42,50,0.7)] shrink-0" />
                        <span className="font-inter text-[14px] text-[#102A32]">{b}</span>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div className="flex flex-col sm:flex-row gap-4">
                  <a href="https://www.helsinkikiropraktiikka.fi/ajanvaraus" className="inline-flex items-center justify-center px-10 py-4 rounded font-inter text-[14px] font-bold tracking-[0.08em] bg-[#123F3D] text-white shadow-[0_4px_16px_rgba(18,63,61,0.18)] hover:-translate-y-0.5 hover:bg-[#00B8B5] transition-all duration-300">
                    Varaa aika
                  </a>
                  <Link to="/#hinnasto" className="inline-flex items-center justify-center px-8 py-4 rounded font-inter text-[14px] font-medium tracking-wide text-[#102A32] border border-[#123F3D]/20 hover:border-[#123F3D]/40 hover:bg-[#123F3D]/[0.04] transition-all duration-300">
                    Katso hinnasto
                  </Link>
                </div>
              </ScrollReveal>
      ...
      <footer className="bg-[#123F3D] px-6 md:px-12 py-10">
        <div className="max-w-[1200px] mx-auto flex flex-col md:flex-row items-center justify-between gap-4">
          <p className="font-inter text-[13px] text-[#C7DEE4]">Helsinki Kiropraktiikka — Kristian Ekström</p>
          <div className="flex items-center gap-6">
            <span className="font-inter text-[12px] text-[#C7DEE4]">040 415 1611</span>
            <span className="font-inter text-[12px] text-[#C7DEE4]">Runeberginkatu 8 C, Helsinki</span>
          </div>
        </div>
      </footer>
    </div>
